...
#    command = <<EOT
#        $vm = Get-AzVM -Name $os_profile.computer_name -ResourceGroupName ${var.resource_group.name}
...
resource "azurerm_network_interface_backend_address_pool_association" "vm-lb-association" {
  count = var.backend_address_pool_id != null ? var.nb_instances : 0
...
#resource "azurerm_mssql_virtual_machine" "SqlIaasExtension" {
#  count                = var.nb_instances
