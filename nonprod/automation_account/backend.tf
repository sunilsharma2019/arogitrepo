...
terraform {
  backend "azurerm" {
    ...
    key                  = "automation_account.terraform.tfstate"  #this key must be unique for each layer
    resource_group_name  = "rg-emoney-Hub-01"   #variable cannot be used in here, it need to hard code in here.
