...
resource "azurerm_key_vault_secret" "vm_pwd" {
  ...
  name = (
    (count.index >= 99) ?
      "${var.vm_hostname}-${count.index + 3}" :
    (count.index >= 9) ?
      "${var.vm_hostname}-0${count.index + 3}" :
      "${var.vm_hostname}-00${count.index + 3}"
...
resource "azurerm_public_ip" "vm_public_ip" {
  ...
  name = (
    (count.index >= 99) ?
      "${var.vm_hostname}-${count.index + 3}-pip" :
    (count.index >= 9) ?
      "${var.vm_hostname}-0${count.index + 3}-pip" :
      "${var.vm_hostname}-00${count.index + 3}-pip"
...
#  name                          = "${var.vm_hostname}-nic-${count.index}"
  name = (
    (count.index >= 99) ?
      "${var.vm_hostname}-nic-${count.index + 3}" :
    (count.index >= 9) ?
      "${var.vm_hostname}-nic-0${count.index + 3}" :
      "${var.vm_hostname}-nic-00${count.index + 3}"
  ...
  ip_configuration {
    name = (
      (count.index >= 99) ?
        "${var.vm_hostname}-ip-${count.index + 3}" :
      (count.index >= 9) ?
        "${var.vm_hostname}-ip-0${count.index + 3}" :
        "${var.vm_hostname}-ip-00${count.index + 3}"
...
resource "azurerm_virtual_machine" "vm-windows" {
  ...
  zones                            = [("${(count.index + 3) % 3 }" == 0) ? "3" : "${(count.index + 3) % 3 }"]
  ...
  storage_os_disk {
    name = (
      (count.index >= 99) ?
        "osdisk-${var.vm_hostname}-${count.index + 3}" :
      (count.index >= 9) ?
        "osdisk-${var.vm_hostname}-0${count.index + 3}" :
        "osdisk-${var.vm_hostname}-00${count.index + 3}"
    )
  ...
  dynamic storage_data_disk {
    ...
    content {
      name = (
        (count.index >= 99) ?
          "${var.vm_hostname}-${count.index + 3}_DataDisk_${storage_data_disk.value}" :
        (count.index >= 9) ?
          "${var.vm_hostname}-0${count.index + 3}_DataDisk_${storage_data_disk.value}" :
          "${var.vm_hostname}-00${count.index + 3}_DataDisk_${storage_data_disk.value}"
      )    
      create_option     = "Empty"
      caching           = var.caching
...
resource "azurerm_backup_protected_vm" "vm" {
  ...
  source_vm_id        = azurerm_virtual_machine.vm-windows[count.index].id
